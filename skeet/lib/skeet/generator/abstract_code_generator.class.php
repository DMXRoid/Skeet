<?
	...
	*/
	...
	abstract class AbstractCodeGenerator {
		...
		protected function generateCode() {
			...
			foreach($this->getTableDescriptionArray() as $tableDescription) {
				...
				foreach($this->getGeneratedCodeTemplateArray() as $generatedCodeTemplateName => $generatedCodeTemplateSettings) {
					...
					if($generatedCodeTemplateSettings["overwrite_existing_file"] || !file_exists(\Skeet\Skeet::getConfig("application_lib_path") . $generatedCodeTemplateSettings["path"] . $tableDescription->getTableName() . $generatedCodeTemplateSettings["file_suffix"])) {
						file_put_contents(\Skeet\Skeet::getConfig("application_lib_path") . $generatedCodeTemplateSettings["path"] . $tableDescription->getTableName() . $generatedCodeTemplateSettings["file_suffix"],ob_get_contents());
					}
					ob_clean();
				}
			}
		...
		public function addGeneratedCodeTemplate($label,$componentName,$outputPath='classes/',$fileSuffix='.class.php',$overwriteExistingFile=false,$additionalSettings=array()) {
